...
Теперь, когда мы знаем больше о типах данных и значениях в JS, можно обратиться к более противоречивой теме: приведении.

Споры о том, чем является приведение типов, полезной фичей или главным багом при разработке языка, не утихают по сей день. Во многих популярных книгах о JS вы могли видеть преобладающее мнение о том, что приведение - нечто магическое, запутанное, чистое зло и вообще ее использование - откровенно плохая идея.

Здесь же, наоборот, вместо того, чтобы бежать прочь от приведения типов, потому что так делают все остальные, или, потому что все эти странности могут вас покусать, стоит наоборот, бежать навстречу тому, что не до конца понятно чтобы узнать о нем как можно больше.

Цель состоит в том, чтобы полностью узнать обо всех преимуществах (да они есть) и недостатках приведения типов, чтобы принять взвешенное решение о целесообразности его использования.

## Конвертируем значения

Преобразование значения из одного типа в другой часто называют "type casting" (кастуем типы), когда оно применяется явно, и "coercion" (приведение), когда оно применено неявно (спровоцировано правилами тех ситуаций, при которых используется это значение).

**Примечание:** Может не до конца очевидно, но JavaScript преобразования всегда в результате отдают одно из значений примитивных калярных типов: `string`, `number`, или `boolean`. Не бывает таких преобразований, результатом которых, будут сложные значения например `object` или `function`. Есть конечно объекты-обертки для скалярных примитивных значений, но это не является настоящим приведением, если быть точным.

Другой способ чтобы отличить эти понятия: "type casting" возникает в строго типизированных языках при компиляции, тогда как "type coercion" приведение типов на лету для динамически типизированных языков.

Тем не менее, в JavaScript, большинство людей при применении обоих преобразований имеют ввиду приведение и чтобы их различать можн использовать внутреннее и внешнее приведение.

Разница очевидна: внешнее приведение, это когда в коде явно и преднамеренно вызывается преобразование типов, тогда как внутреннее приведение появляется в виде побочного эффекта какой-либо преднамеренной операции.


Например, посмотрим на два подхода для преобразования типов:

```js
var a = 42;

var b = a + "";			// внутреннее приведение

var c = String( a );	// внешнее приведение
```
...
Если любое нечисловое значение используется в числовом контексте, где требуется чтбы значение имело тип `number`, например при математических операциях, спецификация ES5 определяет абстрактный оператор `ToNumber` в разделе 9.3.

Например, `true` становится `1` а `false` становятся `0`. `undefined` становится `NaN`, но (что интересно) `null` становится `0`.
